    ...
    *,
    ...
    for col in _UNDERLYING_COLS:
        if col in df.columns:
            S_or_F = S_or_F.combine_first(df[col].where(df[col] > 0))
    ...
    if iv_source == "provided" and "iv_provided" in df.columns:
        sigma = df["iv_provided"].where(df["iv_provided"] > 0)
        if "iv" in df.columns:
            sigma = sigma.combine_first(df["iv"].where(df["iv"] > 0))
    elif "iv" in df.columns:
        sigma = df["iv"].where(df["iv"] > 0)
    ...
    if "right" in df.columns:
        ...
        right = df["option_type"].astype(str).str.upper().map({"CALL": "C", "PUT": "P", "C": "C", "P": "P"})
    else:
    ...
    # ── Valid mask ────────────────────────────────────────────────────────────
    invalid_mask = (
        missing_under | missing_iv | missing_T | bad_right
        | out["K"].isna() | (out["K"] <= 0)
    )
    ...
    invalid_by_reason["missing_strike"] = int((out["K"].isna() | (out["K"] <= 0)).sum())
    summary = {
        "total_rows": n,
