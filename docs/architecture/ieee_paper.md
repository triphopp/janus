...
เวอร์ชันนี้ออกแบบให้เหมาะกับรายงานหรือ paper แบบ IEEE มากกว่า diagram เต็มใน `docs/architecture/pipeline_execution.md` โดยลดจำนวน node, ใช้ข้อความสั้น, ใช้สีแบบ grayscale และจัด layout แนวตั้งเพื่อให้ export แล้ววางได้ในความกว้างประมาณ `\columnwidth`

ไฟล์ Mermaid source แยกอยู่ที่ `docs/architecture/ieee_paper.mmd` และมี SVG draft สำหรับ preview/export อยู่ที่ `docs/assets/images/janus_architecture_ieee.svg`
...
```bash
mmdc -i docs/architecture/ieee_paper.mmd -o docs/assets/images/janus_architecture_ieee.pdf -b white
```
